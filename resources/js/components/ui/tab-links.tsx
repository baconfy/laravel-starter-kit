...
export const TabLinks = ({children, ...props}: ComponentProps<'div'>) => {
    return (
        <div className={cn("inline-flex items-center justify-center md:justify-start rounded-xl bg-accent text-accent-foreground p-2 gap-2", props.className)}>
            {children}
        </div>
    );
};
...
export const TabLink = ({ href, active = false, icon, children, ...props}: ComponentProps<'div'> & { href: string; active: boolean; icon: LucideIcon }) => {
    return (
        <Link href={href} className={cn(
            ...
            "inline-flex items-center justify-center gap-x-1.5 px-4 py-2 rounded-lg",
            {
